...
```text
source/
├── Archive Center.js              active RisuAI adapter source (3.9.11)
...
| `Archive Center 3.4-C.js`, `Archive Center.js.codex-backup-*` | **OBSOLETE.** Copies relative to the active 3.9.11 source. They can run only if someone separately installs them. | No active source import was found. |
...
| Build identity | `AC_BUILD_VERSION` and plugin constants | Go default is `3.9.11-dev`; plugin/package version is `3.9.11`. |

**VERIFIED.** Configuration drift: the repository-root [`.env.example`](.env.example) still contains a `3.7.0-dev` build version while Go defaults and package scripts target 3.9.11. `build-full-package.ps1` rewrites the copied package example, so this is development-template drift, not proof that built packages contain the stale value. Validate all three layers before release.
...
| 2 | Multiple implementation copies drift — **VERIFIED** risk | `Archive Center 3.4-C.js`, backups, and packaged copies can be mistaken for active 3.9.11. | Treat root `Archive Center.js` and `go-service` as active. | Check package copy hash/version against active source. |
...
| 14 | Configuration drift — **VERIFIED** risk | Root `.env.example` says 3.7.0-dev while source/package defaults are 3.9.11-era; package builders rewrite copies. | Validate source defaults, templates, package rewrite, and runtime status together. | Package smoke test and `/version`/`/ready` identity comparison. |
...
- **VERIFIED:** 3.9.11 RisuAI `input`, `beforeRequest`, `afterRequest`, and unload hook registration.
