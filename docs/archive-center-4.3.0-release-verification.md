...
검사일: 2026-09-09. **패키지·소스 검증 완료, 공개는 보류 중**이다.
릴리스 소스는 `51d901bda8173c2d668f64362213375f7eecc028`이며 `v4.3.0` 태그와
`release/4.3.0` 준비 브랜치를 원격에 올렸다. GitHub 릴리스 ID `385346092`는 초안이다.
`main` 갱신은 자동 승인 검토에서 기본 브랜치 변경에 대한 별도 승인이 필요하다는
이유로 차단됐다. main과 공개 latest는 아직 4.2 상태이며, 초안을 정식 공개하지 않았다.
...
Termux arm64 및 `SHA256SUMS-4.3.0.txt`를 초안에 업로드했다.
...
- 위 Windows 검사는 실제 등록된 `POST /update/apply` 생산 경로와 이전 updater를 쓴다.
  ...
  아직 실제 공개 GitHub latest에서 4.3을 받는 실행 증거는 아니다.
...
- main 승인·정식 릴리스 공개 후 실제 공개 latest 다운로드를 통한 관리형 업데이트.
