...
현재 정식 버전은 [4.5.0](archive-center-4.5-release-verification.md)입니다.
아래 명령은 GitHub의 최신 공개 릴리스를 설치합니다.
...
기존 관리형 설치의 업데이트는 설정의 **업데이트 확인 → 지금 업데이트**
경로를 사용한다. 검증된 범위는 [4.5 릴리스 기록](archive-center-4.5-release-verification.md)을
