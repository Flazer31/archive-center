...
func TestCompleteTurnNonCommittedReplacementFailureIsTerminalAndDoesNotReenter(t *testing.T) {
	...
	slog.SetDefault(slog.New(slog.NewJSONHandler(&failureLog, nil)))
	...
	if !strings.Contains(failureLog.String(), "logical_turn_not_current_tail") || firstPayload["persistence_diagnostics"] == nil {
		t.Fatalf("HTTP 200 failure must retain cause in report and response: log=%s body=%v", failureLog.String(), firstPayload)
	}
...
func TestCompleteTurnRerollReplacesCanonicalTailAndDeletesSupersededVector(t *testing.T) {
	for _, kind := range []string{"reroll", "edit"} {
		t.Run(kind, func(t *testing.T) {
			...
			reroll := completeTurnAnchoredAcceptanceTestRequest("session-1", 2, "same user", "rerolled", 2000, generation, "not_streaming", 2, 3, 4)
			...
			if hud["display_mode"] != "notice" || hud["status"] != "completed_with_warning" || hud["severity"] != "warning" ||
				hud["title_key"] != titleKey ||
				hud["message_key"] != messageKey ||
