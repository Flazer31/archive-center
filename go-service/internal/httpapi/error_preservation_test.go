...
func TestMemoryWorkerEmptyWakeScheduleIsNormal(t *testing.T) {
	for _, cause := range []error{store.ErrNotFound, fmt.Errorf("empty schedule: %w", store.ErrNotFound)} {
		f := &workerReadErrorStore{memoryReprocessingDrainStore: &memoryReprocessingDrainStore{memoryAdmissionWorkerStore: &memoryAdmissionWorkerStore{Store: store.NewNoopStore()}}, cause: cause}
		s := &Server{Store: f, RuntimeConfig: RuntimeConfig{Synced: true, CriticTimeoutSec: 2}}
		var logs bytes.Buffer
		prior := slog.Default()
		slog.SetDefault(slog.New(slog.NewJSONHandler(&logs, nil)))
		next := s.processMemoryWorkerWake(context.Background(), "fixture-idle-worker", time.Now())
		slog.SetDefault(prior)
		if !next.IsZero() || logs.Len() != 0 || f.claims != 1 || f.vectorClaims != 2 || f.schedules != 1 {
			t.Fatalf("idle schedule logged/retried or changed drain: next=%v counts=%d/%d/%d logs=%s", next, f.claims, f.vectorClaims, f.schedules, &logs)
		}
	}
}
