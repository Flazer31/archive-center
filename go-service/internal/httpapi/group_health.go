...
func (s *Server) registerConfigRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST /config/view-model", s.handleConfigViewModel)
...
func (s *Server) handleConfigUpdate(w http.ResponseWriter, r *http.Request) {
	...
	for _, role := range []string{"main", "supervisor"} {
		trace[role].(map[string]any)["required_for_sync"] = strings.TrimSpace(extractionStringFromAny(body["publisherApplyMode"])) != "off"
	}
	writeJSON(w, http.StatusOK, map[string]any{
		"status":               "ok",
