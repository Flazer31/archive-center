...
func TestVoyageContextDocumentEmbeddingSplitsLargeSiblingGroupsAndPreservesOrder(t *testing.T) {
	oldClient := proxyHTTPClient
	batchSizes := []int{}
	proxyHTTPClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		var body map[string]any
		...
		groups := sliceFromAny(body["inputs"])
		if len(groups) != 1 {
			t.Fatalf("input groups = %d, want one logical document batch", len(groups))
		}
		chunks := sliceFromAny(groups[0])
		batchSizes = append(batchSizes, len(chunks))
		rows := make([]map[string]any, 0, len(chunks))
		for i, rawChunk := range chunks {
			chunk := extractionStringFromAny(rawChunk)
			if chunk == "" {
				t.Fatal("empty contextualized chunk")
			}
			rows = append(rows, map[string]any{
				"index":     i,
				"embedding": []float64{float64(chunk[0])},
			})
		}
	...
	embeddings, model, err := callDocumentEmbeddings(context.Background(), completeTurnEmbeddingConfig{
		Provider: "voyageai", APIKey: "voyage-key",
		Endpoint: "https://api.voyageai.com/v1", Model: "voyage-context-4",
	}, inputs)
	if err != nil {
		t.Fatalf("callDocumentEmbeddings error: %v", err)
	}
	...
	if !reflect.DeepEqual(batchSizes, []int{3, 1}) {
		t.Fatalf("batch sizes = %#v, want [3 1]", batchSizes)
	}
	if !reflect.DeepEqual(embeddings, []string{"[97]", "[98]", "[99]", "[100]"}) {
