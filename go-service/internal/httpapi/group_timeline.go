...
func (s *Server) handleTimeline(w http.ResponseWriter, r *http.Request) {
	...
	// Explicit metadata projection for input preparation; the ordinary timeline
	// and its filters remain unchanged. Legacy/non-authority stores use the
	// existing list path below, which the caller already understands.
	if r.URL.Query().Get("latestOnly") == "true" && beforeTurn <= 0 && exactTurn == 0 && sid != "" {
		if reader, ok := s.Store.(store.TimelineTurnIndexStore); ok {
			latest, err := reader.LatestTimelineTurnIndex(r.Context(), sid)
			if err == nil {
				writeJSON(w, http.StatusOK, map[string]any{
					"status": "ok", "items": []timelineItem{},
					"meta": map[string]any{"session_id": sid, "latest_turn_index": latest, "read_only": true},
				})
				return
			}
