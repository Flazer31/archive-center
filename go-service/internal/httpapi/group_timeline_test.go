...
type latestTimelineOnlyStore struct {
	store.Store // Any unexpected full-list read fails the test.
	latest      int
	calls       int
}

func (s *latestTimelineOnlyStore) LatestTimelineTurnIndex(ctx context.Context, sid string) (int, error) {
	if sid != "projection-session" {
		panic("wrong session")
	}
	s.calls++
	return s.latest, nil
...
func TestTimelineLatestOnlyDoesNotHydrateHistory(t *testing.T) {
	for _, latest := range []int{0, 7, 200} {
		st := &latestTimelineOnlyStore{latest: latest}
		s := &Server{Store: st}
		r := httptest.NewRecorder()
		s.handleTimeline(r, httptest.NewRequest("GET", "/timeline?sessionId=projection-session&limit=1&latestOnly=true", nil))
		var body struct {
			Items []timelineItem `json:"items"`
			Meta  struct {
				Latest int `json:"latest_turn_index"`
			} `json:"meta"`
		}
		if err := json.Unmarshal(r.Body.Bytes(), &body); err != nil {
			t.Fatal(err)
		}
		if r.Code != 200 || body.Meta.Latest != latest || len(body.Items) != 0 || st.calls != 1 {
			t.Fatalf("projection changed: %d %s calls=%d", r.Code, r.Body, st.calls)
		}
	}
}

func TestTimelineLatestOnlyPreservesExistingStoreReadPath(t *testing.T) {
	s := &Server{Store: &memoryFakeStore{}}
	for _, query := range []string{"", "&latestOnly=true"} {
		r := httptest.NewRecorder()
		s.handleTimeline(r, httptest.NewRequest("GET", "/timeline?sessionId=fixture&limit=1"+query, nil))
		if r.Code != 200 {
			t.Fatalf("%s: %d %s", query, r.Code, r.Body)
		}
	}
}
