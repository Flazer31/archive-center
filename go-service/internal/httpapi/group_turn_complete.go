...
func (s *Server) handleCompleteTurn(w http.ResponseWriter, r *http.Request) {
	...
	diagnosticCtx, finishDiagnostic := diagnosticTurnRequest(r.Context(), "complete_turn", workflowRequestID)
	...
	s.executeCompleteTurnIdempotent(
		...
		func(target http.ResponseWriter) {
			...
			slog.InfoContext(r.Context(), "turn acceptance decided", "request_id", workflowRequestID,
				"enabled", acceptance.Enabled, "accepted", acceptance.Accepted, "reason", acceptance.Reason,
				"requested_turn", req.TurnIndex, "bound_turn", acceptance.BoundTurn,
				"replace_existing", acceptance.ReplaceExisting, "replacement_kind", acceptance.ReplacementKind)
			slog.DebugContext(r.Context(), "turn host position", "request_id", workflowRequestID,
				"user_message_index", acceptance.Observation.UserMessageIndex,
				"input_group_ordinal", acceptance.Observation.UserObservedPairOrdinal,
				"assistant_message_index", acceptance.Observation.MessageIndex,
				"active_message_count", acceptance.Observation.ActiveMessageCount,
				"later_active_turn_messages", acceptance.Observation.LaterActiveTurnMessageCount,
				"previous_bound_turn", acceptance.previousState.TurnIndex)
...
func (s *Server) handleCompleteTurnDecoded(w http.ResponseWriter, r *http.Request, req dto.M4CompleteTurnRequest, sourceAcceptance completeTurnSourceAcceptanceDecision) {
	...
	if sourceAcceptance.Enabled && sourceAcceptance.Accepted && sourceAcceptance.ReplaceExisting {
		...
		if replacementErr := s.replaceCompleteTurnLogicalTail(ctx, sid, req.TurnIndex, userText, assistantText, sourceAcceptance, now); replacementErr != nil {
			replacementDiagnostics := s.completeTurnPersistenceDiagnostics([]string{replacementErr.Error()})
			// This response deliberately remains HTTP 200. Log its actual failure
			// before returning so diagnostics do not mistake it for HTTP success.
			slog.ErrorContext(ctx, "logical turn replacement failed", "request_id", workflowRequestID,
				"turn_index", req.TurnIndex, "code", replacementErr.Code,
				"stage", replacementErr.Stage, "commit_state", replacementErr.CommitState,
				"error", replacementDiagnostics)
			...
			if s.TurnWorkflows != nil && workflowRequestID != "" {
				if replacementErr.RawCommitted {
					...
					s.TurnWorkflows.failWithDetails(
						...
						completeTurnPersistenceHUDDetails(replacementDiagnostics, 0, 0, replacementErr.CommitState, false),
					)
				}
			}
			writeJSON(w, http.StatusOK, map[string]any{
				...
				"persistence_diagnostics": replacementDiagnostics,
	...
	if s.TurnWorkflows != nil && workflowRequestID != "" {
		...
		default:
			...
			if sourceAcceptance.Enabled && sourceAcceptance.Accepted && sourceAcceptance.ReplaceExisting {
				titleKey, messageKey := "turn_hud.notice.reroll_confirmed", "turn_hud.notice.reroll_confirmed_detail"
				if sourceAcceptance.ReplacementKind == "host_observed_edit" {
					titleKey, messageKey = "turn_hud.notice.edit_confirmed", "turn_hud.notice.edit_confirmed_detail"
				}
				s.TurnWorkflows.completeWithNotice(
					workflowRequestID,
					titleKey,
