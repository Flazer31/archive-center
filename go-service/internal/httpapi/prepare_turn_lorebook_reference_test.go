...
func TestPrepareTurnOriginalWorkUsageDoesNotReduceLorebookBudget(t *testing.T) {
	...
	if policy["contract_version"] != "reference_injection_budget.v2" || intFromAny(policy["main_injection_cap_chars"], 0) != 32000 || intFromAny(policy["total_cap_chars"], 0) != 3000 || intFromAny(policy["used_chars"], 0) <= 0 {
	...
	if intFromAny(ledgerLanes["long_term_memory"]["configured_cap_chars"], 0) != 32000 ||
	...
	if intFromAny(ledgerLanes["long_term_memory"]["effective_cap_chars"], 0) != 32000 ||
