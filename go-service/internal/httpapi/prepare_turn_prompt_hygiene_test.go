...
func TestPrepareTurnSupportLinkProjectionIsOneHopPrivateAndNonSelecting(t *testing.T) {
	const privateSentinel = "NEVER_TRACE_PRIVATE_7F2B"
	selection := prepareTurnMemoryLaneSelection{
		VectorRelevant: []store.Memory{{
			ID:          101,
			TurnIndex:   7,
			SummaryJSON: `{"turn_summary":"Mira left the brass key beside Rowan.","characters":["Mira","Rowan"],"items":["brass key"]}`,
		}},
		Relevant: []store.Memory{{
			ID:          102,
			TurnIndex:   7,
			SummaryJSON: `{"turn_summary":"An unrelated event used the same source turn."}`,
		}},
		Deep: []store.Memory{{
			ID:          103,
			TurnIndex:   7,
	...
	trace := buildPrepareTurnSupportLinkProjection(
		...
		[]string{"Mira", "Rowan"},
		[]store.ProtagonistEntityMemory{{
			ID:              401,
			OwnerEntityName: "Mira",
			MemoryText:      privateSentinel,
			SecretGuard:     true,
		}},
		16,
	)
	...
	for key, want := range map[string]bool{
		...
		if got := boolFromAny(trace[key]); got != want {
			t.Fatalf("%s=%v, want %v: %#v", key, got, want, trace)
		}
	}
	if got := intFromAny(trace["hop_limit"], 0); got != 1 {
		t.Fatalf("hop_limit=%d, want 1", got)
	}
	if got := intFromAny(trace["same_source_without_entity_count"], 0); got != 1 {
		t.Fatalf("same-source-only count=%d, want 1: %#v", got, trace)
	}
	...
	if err != nil {
		t.Fatalf("marshal support-link trace: %v", err)
	}
	traceText := string(encoded)
	for _, want := range []string{
		"evidence_memory_source_anchor_match",
		"evidence_kg_source_endpoint_pair_match",
		"evidence_request_entity_match",
	} {
		if !strings.Contains(traceText, want) {
			t.Fatalf("missing %q link: %s", want, traceText)
		}
	}
	if strings.Contains(traceText, privateSentinel) {
		t.Fatalf("private text leaked into trace: %s", traceText)
	}
}

func TestPrepareTurnSupportLinkProjectionDoesNotChangeDelivery(t *testing.T) {
	const summary = "Mira left the brass key beside Rowan."
	build := func(summaryJSON string) prepareTurnInjectionAssembly {
		...
		vectorShadow := map[string]any{
			...
			"search_results": []map[string]any{
				{"source_table": "memories", "source_row_id": "101", "similarity": 0.9},
				{"source_table": "direct_evidence_records", "source_row_id": "201", "similarity": 0.9},
			},
		}
		return buildPrepareTurnInjectionAssembly(
			memories,
			[]store.KGTriple{{
				ID:         301,
				Subject:    "Mira",
			...
			[]store.ProtagonistEntityMemory{{
				ID:              401,
				OwnerEntityName: "Mira",
				MemoryText:      "Mira privately doubts the key exchange.",
				SecretGuard:     true,
			}},
			5, 12000,
	...
	linked := build(`{"turn_summary":"` + summary + `","characters":["Mira","Rowan"],"items":["brass key"]}`)
	sameDeliveryWithoutLinks := build(`{"turn_summary":"` + summary + `"}`)

	linkedTrace := mapFromAny(linked.Counts["support_link_projection"])
	...
	if err != nil {
		t.Fatalf("marshal unlinked trace: %v", err)
	}
	if strings.Contains(string(encodedUnlinked), "evidence_memory_source_anchor_match") {
		t.Fatalf("same source turn became an evidence-memory link without an entity anchor: %s", encodedUnlinked)
	}

	linkedDelivery := []any{
		linked.MemoryText,
		linked.ActualMemoryText,
		linked.ProtectedMemoryText,
		linked.KGText,
		linked.DirectEvidenceText,
		linked.LatestDirectEvidenceText,
		linked.ScopedVerbatimText,
		linked.CharacterRelationshipText,
		linked.CanonRelationshipText,
		linked.Text,
		linked.MemoryDeliveryPlan["final_text"],
	}
	unlinkedDelivery := []any{
		sameDeliveryWithoutLinks.MemoryText,
		sameDeliveryWithoutLinks.ActualMemoryText,
		sameDeliveryWithoutLinks.ProtectedMemoryText,
		sameDeliveryWithoutLinks.KGText,
		sameDeliveryWithoutLinks.DirectEvidenceText,
		sameDeliveryWithoutLinks.LatestDirectEvidenceText,
		sameDeliveryWithoutLinks.ScopedVerbatimText,
		sameDeliveryWithoutLinks.CharacterRelationshipText,
		sameDeliveryWithoutLinks.CanonRelationshipText,
		sameDeliveryWithoutLinks.Text,
		sameDeliveryWithoutLinks.MemoryDeliveryPlan["final_text"],
	}
