...
func TestNewServerIgnoresMilvusStubFlag(t *testing.T) {
	cfg := config.Default()
	cfg.MilvusStubEnabled = true
	cfg.MilvusLitePath = "/tmp/test.db"
	srv := NewServer(cfg)
	...
	// Milvus flags are historical no-ops in the active runtime. Without
	// ChromaDB, the server must stay on the fake vector store.
	_, err := srv.Vector.Search(context.Background(), "s1", []float32{0.1}, 1, "")
	if !errors.Is(err, vector.ErrNotFound) {
		t.Errorf("expected fake store to return ErrNotFound, got %v", err)
	}
...
func TestNewServerIgnoresMilvusSDKFlag(t *testing.T) {
	cfg := config.Default()
	...
	// Milvus SDK is no longer an active runtime selection path.
	_, err := srv.Vector.Search(context.Background(), "s1", []float32{0.1}, 1, "")
	if !errors.Is(err, vector.ErrNotFound) {
		t.Errorf("expected fake store to return ErrNotFound, got %v", err)
	}
