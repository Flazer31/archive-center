...
func (d *dualWriteStore) LatestTimelineTurnIndex(ctx context.Context, chatSessionID string) (int, error) {
	reader, ok := d.primary.(TimelineTurnIndexStore)
	if !ok {
		return 0, ErrNotEnabled
	}
	return reader.LatestTimelineTurnIndex(ctx, chatSessionID)
}
