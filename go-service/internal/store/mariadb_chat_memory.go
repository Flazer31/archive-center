...
func (m *mariadbStore) LatestTimelineTurnIndex(ctx context.Context, chatSessionID string) (int, error) {
	if err := m.ensureDB(); err != nil {
		return 0, err
	}
	var latest int
	err := m.db.QueryRowContext(ctx, `
		SELECT GREATEST(0, COALESCE(MAX(latest), 0)) FROM (
	...
	`, chatSessionID, chatSessionID, chatSessionID, chatSessionID, chatSessionID).Scan(&latest)
	return latest, err
}
