...
func TestMariaDBNextMemoryReprocessingWakeAtEmptyAndReadFailure(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	m := &mariadbStore{db: db}
	mock.ExpectQuery(`(?s)SELECT MIN\(CASE.*FROM memory_reprocessing_jobs`).WillReturnRows(sqlmock.NewRows([]string{"next_wake_at"}).AddRow(nil))
	next, err := m.NextMemoryReprocessingWakeAt(context.Background())
	if !next.IsZero() || !errors.Is(err, ErrNotFound) {
		t.Fatalf("empty schedule contract changed: %v %v", next, err)
	}
	cause := errors.New("fixture SQL read failed")
	mock.ExpectQuery(`(?s)SELECT MIN\(CASE.*FROM memory_reprocessing_jobs`).WillReturnError(cause)
	_, err = m.NextMemoryReprocessingWakeAt(context.Background())
...
func TestMariaDBLogicalReplacementInvalidatesDescendantsAndQueuesVectorDeletes(t *testing.T) {
	...
	mock.ExpectExec("UPDATE memory_source_revisions").
		...
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("INSERT INTO memory_source_revisions").WillReturnResult(sqlmock.NewResult(12, 1))
	mock.ExpectExec(`(?s)INSERT INTO character_events.*SELECT state.chat_session_id`).WithArgs(source.ChatSessionID, source.TurnIndex).WillReturnResult(sqlmock.NewResult(0, 0))
