...
func TestLatestTimelineQueryPreservesFiveTimelineSources(t *testing.T) {
	...
	// Exact projections: no text/vector hydration, no newer eligibility filters,
	// and evidence uses turn_anchor rather than its different source range.
	mock.ExpectQuery(`(?s)SELECT GREATEST\(0, COALESCE\(MAX\(latest\), 0\)\) FROM \(\s*SELECT MAX\(turn_index\) AS latest FROM chat_logs WHERE chat_session_id = \?\s*UNION ALL SELECT MAX\(turn_index\) FROM memories WHERE chat_session_id = \?\s*UNION ALL SELECT MAX\(turn_anchor\) FROM direct_evidence_records WHERE chat_session_id = \?\s*UNION ALL SELECT MAX\(source_turn\) FROM kg_triples WHERE chat_session_id = \?\s*UNION ALL SELECT MAX\(to_turn\) FROM episode_summaries WHERE chat_session_id = \?\s*\) AS timeline_turns`).
		WithArgs("session", "session", "session", "session", "session").
		WillReturnRows(sqlmock.NewRows([]string{"latest"}).AddRow(210))
	got, err := st.LatestTimelineTurnIndex(context.Background(), "session")
	if err != nil || got != 210 {
