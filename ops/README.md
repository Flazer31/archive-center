...
- `build-full-package.ps1 -ChromaRuntime <dir-or-zip> -Zip -ForceRefresh`: creates the standard Windows distribution layout under `_dist`, builds the Go backend binaries, copies `Archive Center.js`, prompts, migrations, notices, and package scripts, and verifies the ChromaDB runtime payload. MariaDB is never copied into the Archive Center package.
...
Note: `start-windows-live.ps1` is retained for source/dev verification. For packaged verification, use the full package generated by `build-full-package.ps1`, or the lighter live-test pack generated by `build-live-test-pack.ps1`. Both paths target MariaDB + ChromaDB.
