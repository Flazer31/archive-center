...
- `install-windows.ps1 -InstallMariaDBRuntime -InstallDir <path> -ExternalOperationTimeoutSeconds <seconds>`: downloads the pinned official MariaDB 12.3.2 Windows ZIP, verifies its SHA-256, and installs it outside the Archive Center package. The timeout is caller-selected; no hidden deadline is substituted. No administrator rights or system service registration is required.
