...
function Set-CopiedPackageVersionText([string]$Root, [string]$PackageVersion) {
    $version = if ([string]::IsNullOrWhiteSpace($PackageVersion)) { "4.0.5" } else { $PackageVersion.Trim() }
...
$packageVersionLabel = if ([string]::IsNullOrWhiteSpace($PackageVersion)) { "4.0.5" } else { $PackageVersion.Trim() }
