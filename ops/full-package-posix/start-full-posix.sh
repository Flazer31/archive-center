...
run_external() {
	...
	(
		# Waiting on a background sleep lets the guard handle cancellation now,
		# rather than leave a foreground sleep holding the diagnostic pipes open.
		external_sleep_pid=
		external_guard_cancelled=false
		trap '
			external_guard_cancelled=true
			if [ -n "$external_sleep_pid" ]; then
				env kill -KILL "$external_sleep_pid" >/dev/null 2>&1 || true
			fi
		' HUP INT TERM
		(exec sleep "$EXTERNAL_OPERATION_TIMEOUT_SECONDS") &
		external_sleep_pid=$!
		# Cancellation can arrive between starting sleep and recording its PID.
		if [ "$external_guard_cancelled" = true ]; then
			env kill -KILL "$external_sleep_pid" >/dev/null 2>&1 || true
		fi
		wait "$external_sleep_pid" 2>/dev/null || true
		if [ "$external_guard_cancelled" = true ]; then
			wait "$external_sleep_pid" >/dev/null 2>&1 || true
			exit 0
		fi
...
cleanup() {
	...
	if [ -n "${EXTERNAL_OPERATION_PID:-}" ]; then
		kill "$EXTERNAL_OPERATION_PID" >/dev/null 2>&1 || true
		wait "$EXTERNAL_OPERATION_PID" >/dev/null 2>&1 || true
		EXTERNAL_OPERATION_PID=
	fi
...
# Bootstrap can also be interrupted while an external installer is running.
