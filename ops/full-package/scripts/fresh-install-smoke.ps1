...
if (Test-Path -LiteralPath $managedManifestPath -PathType Leaf) {
    try {
        ...
        foreach ($requiredManagedPath in @("bin/archive-center-go.exe", "bin/archive-center-updater.exe", "bin/mariadb-schema.exe", "scripts/start-full-windows.ps1", "01_start_archive_center_windows.bat", "tools/install-windows.ps1", "PACKAGE_MIGRATION_UPDATE.json", "Archive Center.js")) {
...
if (Test-Path -LiteralPath $launcherScriptPath -PathType Leaf) {
    ...
    if ($launcherScriptText -notmatch '(?s)if \(\$pendingApplyStatus -eq "applied_pending_health"\).*?\}\s*else\s*\{\s*\$backendProcess\s*=\s*Start-ArchiveChildProcess.*?Wait-ArchiveBackendLifetime') {
