...
# Archive Center 2.1 Linux installer preflight skeleton.
# This script does not install packages, initialize MariaDB, or create ChromaDB runtime state.

usage() {
	cat <<'EOF'
Usage:
  install-linux.sh --preflight [--out PATH] [--data-dir PATH]
...
if [ -n "${ARCHIVE_CENTER_DATA_DIR:-}" ]; then
	...
	DATA_DIR="${XDG_DATA_HOME}/archive-center"
else
	DATA_DIR="${HOME:-.}/.local/share/archive-center"
fi
...
if printf '%s %s' "$UNAME_S" "$UNAME_O" | grep -qi 'Android'; then
	IS_LINUX=false
	append_failure "android_detected_use_install_termux"
fi
if [ "$IS_LINUX" != "true" ]; then
	append_failure "not_running_on_linux"
fi
...
if [ "$PKG_MANAGER" = "none" ]; then
	append_warning "linux_package_manager_not_found"
fi
...
if has_cmd systemctl; then
	...
	append_warning "systemctl_not_found_user_mode_launcher_required"
fi
...
if [ "$MARIADB_BUNDLED_AVAILABLE" = "true" ]; then
	...
	MARIADB_PROVIDER_MODE="installer_package_required"
	MARIADB_REQUIRED_ACTION="installer/bootstrap must install and initialize MariaDB through package manager $(printf '%s' "$PKG_MANAGER")"
	append_warning "mariadb_provider_installer_managed_required"
else
...
if [ -n "$FAILURES" ]; then
	...
	FALLBACK_PROFILE="linux_light_or_managed_dependency_profile_required"
else
	SUPPORT_LEVEL="green"
	PREFLIGHT_STATUS="ok"
	FALLBACK_PROFILE="none"
...
REPORT=$(cat <<EOF
{
  ...
  "linux": {
    "detected": $(json_bool "$IS_LINUX"),
    "package_manager": $(json_string "$PKG_MANAGER"),
    "systemd_available": $(json_bool "$SYSTEMD_AVAILABLE")
  },
  "go_backend": {
