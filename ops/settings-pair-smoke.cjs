...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(()=>window.transport.some(x=>x.path==='/config/update'&&x.status===200));
    const sync=await page.evaluate(()=>window.transport.filter(x=>x.path==='/config/update').at(-1));
    ...
    if(process.argv.includes('--diagnostics')) {
      ...
      assert.deepEqual(errors,[]);
      console.log('PASS: diagnostic UI, online/offline downloads, key redaction, persisted host errors after reload.');
    }
